...
export function useSdkClients(organizationSlug: string, projectKey: string) {
  ...
  const clientsQueryKey = useMemo(() => {
    return ["sdkClients", organizationId, projectId] as const;
  ...
  const clientsQuery = useQuery({
    ...
    queryFn: () => {
      ...
      return sdkClientsService.listByProject(organizationId, projectId);
    },
  });
  ...
  return {
    ...
    organizationId,
    projectId,
    sdkClients: (clientsQuery.data ?? []) as SdkClientDto[],
