...
import type { CreateProcessDefinitionDto } from "@/types/admin.types";
...
function getErrorMessage(error: unknown) {
  ...
  return "Не удалось создать процесс.";
}

const processTypeOptions: { value: ProcessType; label: string }[] = [
  { value: ProcessType.Session, label: "Сессия" },
  { value: ProcessType.Order, label: "Заказ" },
  { value: ProcessType.Checkout, label: "Оформление" },
  { value: ProcessType.Ticket, label: "Тикет" },
  { value: ProcessType.Custom, label: "Пользовательский" },
];
...
export function CreateProcessDialog({
  ...
  const [type, setType] = useState<ProcessType>(ProcessType.Custom);
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        <div>
          <CardTitle>Создать процесс</CardTitle>
          <CardDescription>Название, ключ, тип и опциональное описание.</CardDescription>
        </div>
        <Button
          variant="ghost"
          size="sm"
      ...
      <div className="px-5 pb-5">
        <div className="space-y-4">
          ...
          <label className="block">
            ...
            <select
              value={type}
              onChange={(e) => setType(Number(e.currentTarget.value) as ProcessType)}
          ...
          <label className="block">
            ...
            <Input
              ...
              placeholder="Коротко о процессе"
              className="mt-2"
            />
          </label>
